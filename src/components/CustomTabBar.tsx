...
// Flip to false to silence haptics without removing the calls.
...
export function CustomTabBar({ state, descriptors, navigation }: BottomTabBarProps) {
  ...
  return (
    <BlurView
      ...
      style={[styles.blur, { borderTopColor: colors.line }]}
    >
      <View style={[styles.row, { paddingBottom: Math.max(insets.bottom, 8) }]}>
        {state.routes.map((route, index) => {
          ...
          const activeColor = isDark ? colors.amber500 : colors.navy800;
          const iconColor   = isFocused ? activeColor : colors.ink400;
            ...
            return (
              <View key={route.key} style={styles.fabContainer}>
                ...
                <View style={styles.fabHalo}>
                  ...
                  >
                    {TAB_ICONS['nuevo'](colors.navy800)}
                  </Pressable>
                </View>
              </View>
            );
...
const styles = StyleSheet.create({
  ...
  fabContainer: {
    flex: 1,
    alignItems: 'center',
    // FAB overflows upward; give enough height so it isn't clipped by the row.
    marginBottom: 4,
  },
  fabHalo: {
    borderRadius: 24,
    ...
    shadowRadius: 20,
    elevation: 10,
    marginTop: -18, // lift FAB above the bar
  },
  fab: {
    width: 54,
    height: 54,
